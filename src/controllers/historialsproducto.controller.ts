...
import { Historicoaccproducto } from "../models/historicoaccproducto"
import { Producto } from "../models/producto";


export const getHistorials = async (_req:Request, res: Response) => {
    try {
        const listaHistorials= await Historicoaccproducto.findAll();
...
export const getHistorial = async (req:Request, res: Response) => {
    try {         
        const { id } = req.params;
        const historial = await Historicoaccproducto.findByPk(id);
...
export const getHistorialUsuario = async (req:Request, res: Response) => {
    try {         
        const { id } = req.params;
        const historial = await Historicoaccproducto.findAll({
            where:{
                'uid':id
            },
            include: [
...
export const createHistorial = async (req:Request, res: Response) => {
    try {
        ...
        const newHistorial = await Historicoaccproducto.create(req.body);
...
export const deleteHistorial = async (req:Request, res: Response) => {
    try {
        const { id } = req.params;
        await Historicoaccproducto.destroy({
            where: {
                id,
            }
        });
